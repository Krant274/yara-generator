...
class FeatureSynthesizer:
    ...
    SCORE_PATTERNS = {
        ...
        'file_extensions': (r'(\.exe|\.pdb|\.scr|\.log|\.cfg|\.txt|\.dat|\.msi|\.com|\.bat|\.dll|\.vbs|\.tmp|\.sys|\.ps1|\.vbp|\.hta|\.lnk)', 4),
        
        # System keywords
        'system_keywords': (r'(cmd\.exe|system32|users|Documents and|SystemRoot|Grant|hello|password|process|log)', 5),
        ...
        'hacktool_keywords': (r'(scan|sniff|poison|intercept|fake|spoof|sweep|dump|flood|inject|forward|vulnerable|credentials|creds|coded|p0c|Content|host)', 5),
        
        # Network keywords
        'network_keywords': (r'(address|port|listen|remote|local|process|service|mutex|pipe|frame|key|lookup|connection)', 3),
        ...
        # URL
        'url_pattern': (r'(%[a-z][:\-,;]|\\\\%s|\\\\[A-Z0-9a-z%]+\\[A-Z0-9a-z%]+)', 2.5),
        
        # Certificates (reduce score)
        'certificate_generic': (r'(thawte|trustcenter|signing|class|crl|CA|certificate|assembly)', -4),
        ...
        'filesystem_elements': (r'(cmd|com|pipe|tmp|temp|recycle|bin|secret|private|AppData|driver|config)', 3),
        
        # Programming
        'programming': (r'(execute|run|system|shell|root|cimv2|login|exec|stdin|read|process|netuse|script|share)', 3),
        
        # Credentials
        'credentials': (r'(user|pass|login|logon|token|cookie|creds|hash|ticket|NTLM|LMHASH|kerberos|spnego|session|identif|account|auth|privilege)', 3),
        
        # RATs / Malware
        'rat_malware': (r'(spy|logger|dark|cryptor|RAT\b|eye|comet|evil|xtreme|poison|meterpreter|metasploit|/veil|Blood)', 5),
        
        # User profiles
        'user_profiles': (r'[\\](users|profiles|username|benutzer|Documents and Settings|Utilisateurs|Utenti|Usuários)[\\]', 3),
        ...
        # Base64
        'base64_long': (r'^(?:[A-Za-z0-9+/]{4}){30,}(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$', 7),
        
        # Base64 Executables
        'base64_executable': (r'(TVqQAAMAAAAEAAAA//8AALgAAAA|TVpQAAIAAAAEAA8A//8AALgAAAA|TVqAAAEAAAAEABAAAAAAAAAAAAA|TVoAAAAAAAAAAAAAAAAAAAAAAAA|TVpTAQEAAAAEAAAA//8AALgAAAA)', 5),
        
        # Malicious intent
        'malicious_intent': (r'(loader|cmdline|ntlmhash|lmhash|infect|encrypt|exec|elevat|dump|target|victim|override|traverse|mutex|pawnde|exploited|shellcode|injected|spoofed|dllinjec|exeinj|reflective|payload|inject|back conn)', 5),
        
        # Privileges
        'privileges': (r'(administrator|highest|system|debug|dbg|admin|adm|root) privilege', 4),
        ...
        'malware_special': (r'(Management Support Team1|/c rundll32|DTOPTOOLZ Co.|net start|Exec|taskkill)', 4),
        
        # Powershell
        'powershell': (r'(bypass|windowstyle | hidden |-command|IEX |Invoke-Expression|Net\.Webclient|Invoke[A-Z]|Net\.WebClient|-w hidden |-encoded|encodedcommand| -nop |MemoryLoadLibrary|FromBase64String|Download|EncodedCommand)', 4),
        
        # WMI
        'wmi': (r'( /c WMIC)', 3),
        
        # Windows Commands
        'windows_commands': (r'( net user | net group |ping |whoami |bitsadmin |rundll32\.exe javascript:|schtasks\.exe /create|/c start )', 3),
        
        # Malware names - high score
        'malware_name': (r'(ransomware|locky|cryptolocker|cryptowall|wannacry|petya|notpetya|emotet|trickbot|cobalt|icedid|qakbot|redline|raccoon|asyncrat|remcos|_formbook|poisonivy|keylogger|trojan|backdoor|dropper|loader|stub|keylog|infostealer|spyware|adware)', 10),
        
        # Encryption related
        'encryption': (r'(encrypt|decrypt|crypt|aes|rc4|rsa|des|key|cipher|iv|salt|crypto|hash|password|credential|credential|dump|pwdump|lsass)', 5),
    ...
    def _load_pestudio_strings(self) -> Dict[str, Dict]:
        ...
        xml_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "3rdparty", "strings.xml")
        ...
        try:
            ...
            for category in ["av", "folder", "os", "reg", "guid", "sddl", "ext", "agent", "oid", "priv", "regex"]:
                for elem in tree.findall(f".//{category}"):
                    text = elem.text
                    if text:
                        pestudio[category][text.lower()] = 1
            
            print(f"[*] Loaded PEStudio blacklist: {len(pestudio['strings'])} strings, {len(pestudio['av'])} av, {len(pestudio['reg'])} reg, etc.")
    ...
    def _get_pestudio_score(self, value: str) -> Tuple[int, str]:
        ...
        for category in ["av", "reg", "folder", "os", "guid", "ext", "agent", "oid", "priv", "regex"]:
    ...
    def _load_whitelist_from_dbs(self, dbs_dir: str = None, auto_download: bool = True) -> Dict:
