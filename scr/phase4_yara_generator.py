...
class YARAGenerator:
    ...
    def generate(
        self, feature_sets: Dict, analyses: Optional[List] = None
    ) -> List[YARARule]:
    ...
    def _generate_super_rules(
        self, feature_sets: Dict, analyses: List
    ) -> List[YARARule]:
    ...
    def _generate_hex_string_rules(
        self, feature_sets: Dict, analyses: Optional[List] = None
    ) -> List[YARARule]:
        ...
        hex_to_use = [
            s for s in hex_strings[:20] if s.frequency >= 0.7 and len(s.value) <= 100
        ][:10]
        ...
        if len(hex_to_use) >= 2:
            ...
            rule = YARARule(
                ...
                strings=[
                    {"type": "hex", "value": s.value, "modifiers": ""}
                    for s in hex_to_use
                ],
                condition=condition,
            )
    ...
    def _generate_base64_rules(
        ...
        if len(high_quality_b64) >= 2:
            ...
            rule = YARARule(
                ...
                strings=[
                    {"type": "string", "value": s.value, "modifiers": "base64"}
    ...
    def _generate_reversed_string_rules(
        self, feature_sets: Dict, analyses: Optional[List] = None
    ) -> List[YARARule]:
        ...
        if len(high_quality_reversed) >= 2:
            ...
            rule = YARARule(
                ...
                strings=[
                    {"type": "string", "value": s.value, "modifiers": "ascii"}
        ...
        for a in analyses:
            if isinstance(a, dict):
                ...
                if isinstance(static, dict):
                    ...
                    size = (
                        getattr(static, "file_size", 0)
                        if hasattr(static, "file_size")
                        else 0
                    )
            else:
